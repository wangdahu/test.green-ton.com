...
final class Application {
	...
	public static function routeToCm($url_array = array()){
		...
		$app = isset($url_array['app']) ? $url_array['app'] : self::$_config['route']['default_app'];
		...
		if($app){
			...
			$model_file = CONTROLLER_PATH.'/'.$app.'/'.$model.'Model.php';
		}else{
			$controller_file = CONTROLLER_PATH.'/'.$controller.'Controller.php';
			$model_file = CONTROLLER_PATH.'/'.$model.'Model.php';
		}
		...
		self::$_lib['app'] = $app;
		self::$_lib['controller'] = $controller;
		self::$_lib['action'] = $action;
